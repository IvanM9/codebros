const ViewConsultors = ({ consultors }) => {
  return (
    ...
    <p className='absolute top-16 lg:top-5 text-2xl font-bold text-gray-900 mb-10'>Todos los Colsultores</p>

    <div className='w-full flex flex-col gap-8 mt-14'>
      {consultors.map((consultor) => (
        <div
          key={consultor.id}
          className='w-full mx-auto max-w-lg rounded-lg border border-stone bg-gray-200 shadow-lg sm:p-6 lg:p-8'
        >
            ...
            {consultor.skills && consultor.skills.length > 0 && (
              <div className='mb-4 col-span-2 grid grid-cols-2 gap-x-4'>
                ...
                {consultor.skills.map((skill) => (
                  <p key={skill.id} className='text-gray-600 mb-1 capitalize col-span-2'>
                    {skill.name}
                  </p>
                ))}
              </div>
